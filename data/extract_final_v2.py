...
for url, slug, titulo_default in urls:
    ...
    # Buscar en columnas del hero
    for column in soup.find_all("div", class_=re.compile("elementor-column")):
        h2 = column.find("h2", class_=re.compile("elementor-heading-title"))
        if h2:
            label = h2.get_text(strip=True).lower()
            # Buscar texto en cualquier descendiente
            container = column.find("div", class_="elementor-widget-container")
            if container:
                text = container.get_text(separator=" ", strip=True)
                # Remover el label
                body = text.replace(h2.get_text(strip=True), "").strip()
                if "duraci" in label and len(body) > 0:
                    duracion = body
                elif label == "título" and len(body) > 0:
                    titulo_grado = body
                elif "sede" in label and len(body) > 0:
                    sede = body
    ...
    if main_content:
        ...
        while i < len(widgets):
            ...
            # Si es un widget con h2 label y p en el mismo widget
            if h2:
                label = h2.get_text(strip=True).lower()
                container = widget.find("div", class_="elementor-widget-container")
                if container:
                    # Obtener texto sin el h2
                    texts = []
                    for elem in container.find_all(string=True, recursive=True):
                        parent = elem.parent
                        if parent and parent.name != "h2" and not parent.find_parent("h2"):
                            t = str(elem).strip()
