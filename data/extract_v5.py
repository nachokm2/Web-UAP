...
for url, slug, titulo_default in urls:
    ...
    if main_content:
        ...
        while i < len(widgets):
            ...
            # Si es un widget con h2 label y contenido largo
            if h2:
            ...
            # Si es un ul con label de sección
            if ul:
                items = [li.get_text(strip=True) for li in ul.find_all("li")]
                if items:
                    section_name = items[0].lower()
                    
                    # CASO 1: El contenido está en los items del mismo ul (items[1], items[2], etc.)
                    ...
                    # CASO 2: El contenido está en el siguiente widget (icon-list o text-editor)
                    else:
                        # Buscar el siguiente widget que NO sea solo un label
                        j = i + 1
                        while j < len(widgets):
                            next_widget = widgets[j]
                            next_ul = next_widget.find("ul")
                            next_h2 = next_widget.find("h2")
                            
                            # Si es un widget icon-list con contenido largo
                            if next_ul:
                                ...
                                if next_items and len(next_items[0]) > 50:
                                    ...
                                    if section_name == "descripción":
                                        ...
                                        definicion_profesional = clean_text(content)
                                    break
                                elif next_items and len(next_items[0]) < 50:
                                    # Si es otro label, saltarlo
                                    j += 1
                                    continue
                                else:
                                    j += 1
                                    continue
                            # Si es un widget con h2 y contenido
                            elif next_h2:
                                j += 1
                                continue
                            # Si es text-editor con contenido largo
                            else:
